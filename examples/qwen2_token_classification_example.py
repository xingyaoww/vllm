#!/usr/bin/env python3
# SPDX-License-Identifier: Apache-2.0

"""
Example script for using vLLM with Qwen2ForTokenClassification.

This example demonstrates how to use the native vLLM implementation of 
Qwen2ForTokenClassification for token-level classification tasks.
"""

import argparse
import json

import torch
from transformers import AutoTokenizer

from vllm import LLM


def parse_args():
    parser = argparse.ArgumentParser(
        description="Example script for Qwen2ForTokenClassification with vLLM")
    parser.add_argument(
        "--model", 
        type=str, 
        default="Qwen/Qwen2-7B-Instruct", 
        help="Model name or path")
    parser.add_argument(
        "--tokenizer", 
        type=str, 
        help="Tokenizer name or path")
    parser.add_argument(
        "--tensor-parallel-size", 
        "-tp", 
        type=int, 
        default=1)
    parser.add_argument(
        "--input-text",
        type=str,
        default="The quick brown fox jumps over the lazy dog.")
    parser.add_argument(
        "--num-labels",
        type=int,
        default=2,
        help="Number of labels for token-level classification")
    return parser.parse_args()


def main(args):
    # Initialize the tokenizer
    tokenizer_name = args.tokenizer or args.model
    tokenizer = AutoTokenizer.from_pretrained(tokenizer_name)
    
    # Initialize the LLM for token-level classification
    llm = LLM(
        model=args.model,
        tokenizer=tokenizer_name,
        tensor_parallel_size=args.tensor_parallel_size,
        trust_remote_code=True,
        task="token_reward",  # Use token_reward task for token classification
        model_config={"num_labels": args.num_labels},
    )
    
    # Tokenize the input text
    inputs = tokenizer(args.input_text, return_tensors="pt")
    input_ids = inputs["input_ids"].tolist()[0]
    
    # Get token-level predictions
    outputs = llm.encode(args.input_text)
    
    # Process and print the results
    print(f"Input text: {args.input_text}")
    print("\nToken-level predictions:")
    
    # Get the token-level logits
    token_logits = outputs[0].data
    
    # Convert logits to probabilities
    token_probs = torch.nn.functional.softmax(
        torch.tensor(token_logits), dim=-1)
    
    # Print token-by-token results
    tokens = tokenizer.convert_ids_to_tokens(input_ids)
    
    for i, (token, probs) in enumerate(zip(tokens, token_probs)):
        label_probs = {f"Label {j}": float(p) for j, p in enumerate(probs)}
        predicted_label = probs.argmax().item()
        
        print(
            f"Token {i}: '{token}' - "
            f"Predicted: Label {predicted_label} - "
            f"Probabilities: {json.dumps(label_probs)}")


if __name__ == "__main__":
    args = parse_args()
    main(args)